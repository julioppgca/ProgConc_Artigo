...
function handler (req, res) {
	...
	//console.log(req);
	console.log(req.url);
	if (req.url === '/node_modules/chart.js/dist/Chart.bundle.js') {
        res.writeHead(200, {'Content-Type': 'application/javascript'});
        fs.createReadStream('node_modules/chart.js/dist/Chart.bundle.js').pipe(res);
        return;
    }
	if (req.url === '/node_modules/chart.js/samples/utils.js') {
        res.writeHead(200, {'Content-Type': 'application/javascript'});
...
io.sockets.on('connection', function (socket) {
    ...
    prog=child.exec('./serial'), function (error, stdout, stderr) {
    	if (error) {
